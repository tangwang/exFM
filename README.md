...
可靠：针对FM对内置的adamW/FTRL等优化器和batch训练进行了精心的调参，使用默认配置可以得到一个不错的baseline。已经在千万日活的业务体中用于召回和粗排，增加精排功能的[deepFM](https://github.com/tangwang/deepFM)也在开发中。

高效：训练速度相比TensorFlow/pytorc实现的FM块很多倍。
