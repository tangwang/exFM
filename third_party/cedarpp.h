...
public:
    ...
    struct ninfo {
        // x1.5 update speed; +.25 % memory (8n -> 10n)
        ...
        if (offset >= sizeof(int)) {
            ...
            if (from >> 32) {
                ...
                for (npos_t offset_ = static_cast<npos_t>(-_array[from].base);
                        offset_ < offset;) {
                    from = static_cast<size_t>(_follow(from, static_cast<uchar>(_tail[offset_]), cf));
                    ++offset_;
            ...
            if (pos == len || tail[pos] == '\0') {
                const int to = _follow(from, 0, cf);
                if (pos == len)
                    return _array[to].value += val; // set value on tail
...
#ifdef USE_EXACT_FIT
            _quota += needed > *_length || needed > MAX_ALLOC_SIZE ? needed : (*_length >= MAX_ALLOC_SIZE ? MAX_ALLOC_SIZE : *_length);
#else
    ...
    void shrink_tail()
    {
        ...
        const size_t length_ = static_cast<size_t>(*_length) - static_cast<size_t>(*_length0) * (1 + sizeof(value_type));
        ...
        for (int to = 0; to < _size; ++to) {
            ...
            if (n.check >= 0 && _array[n.check].base != to && n.base < 0) {
                ...
                do
                    tail[i] = tail_[i];
                while (tail[i++]);
                *reinterpret_cast<value_type *>(&tail[i]) = *reinterpret_cast<const value_type *>(&tail_[i]);
             ...
             const size_t offset = 0, size_t size_ = 0)
    {
        ...
        if (std::fseek(fp, static_cast<long>(offset), SEEK_SET) != 0)
            return -1;
        int len = 0;
        if (std::fread(&len, sizeof(int), 1, fp) != 1)
            return -1;
        const size_t length_ = static_cast<size_t>(len);
        if (size_ <= offset + length_)
            return -1;
        // set array
        clear(false);
    ...
    {
        uchar c = 0;
        if (const int offset = static_cast<int>(from >> 32)) {
            // on tail
            if (root >> 32)
...
private:
    ...
    void _initialize()
    {
        // initilize the first special block
    ...
    int _resolve(npos_t &from_n, const int base_n, const uchar label_n, T &cf)
    {
        ...
        for (const uchar *p = first; p <= last; ++p) {
            // to_ => to
    ...
    void _test(const npos_t from = 0) const
    {
        ...
        do {
            ...
            if (c)
                _test(static_cast<npos_t>(base ^ c));
        }
